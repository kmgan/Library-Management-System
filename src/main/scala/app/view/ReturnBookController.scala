...
class ReturnBookController(
  ...
  def returnBook(action: ActionEvent): Unit = {
    if (bookIdField != null && studentIdField != null) {
      ...
      if (bookId.isEmpty || studentId.isEmpty) {
        ...
        val issuedBook = AllIssuedBook.getById(bookId)
        issuedBook match {
          case Some(returnedBook) =>
            val confirmation = new Alert(Alert.AlertType.Confirmation) {
              title = "Confirm Return"
              headerText = s"Return book $bookId from student $studentId?"
            }

            val result = confirmation.showAndWait()
            if (result.contains(ButtonType.OK)) {
              returnedBook.delete() match {
                case Success(_) =>
                  MainApp.allIssuedBookData -= returnedBook
                ...
                case Failure(e) =>
                  val alert = new Alert(Alert.AlertType.Warning) {
                    ...
                    contentText = "Database problem failed to delete record"
                  }.showAndWait()
              }
            }

          case None =>
            showErrorMessage("Issued book not found!")
        }
      }
    }
  }
